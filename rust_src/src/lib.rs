...
fn query_server_full(py: Python<'_>, ip: String, port: u16) -> PyResult<(String, String, String, Vec<(String, String)>)> {
...
#[pyfunction]
#[pyo3(signature = (ip, port))]
fn ping_server(py: Python<'_>, ip: String, port: u16) -> PyResult<(String, String, String)> {
...
#[pymodule]
fn dmtl_net(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(query_server_full, m)?)?;
    m.add_function(wrap_pyfunction!(ping_server, m)?)?;
    Ok(())
}